...
2. Apply only the numbered migrations you have not already applied, in ascending order, through `009_private_credentials_and_archive_guards.sql`. Each numbered upgrade from 002 onward is transactional and can be rerun. Use the current copy of 007: it now handles existing dependent constraints correctly. Migration 007 will reject existing cross-session category/option/prompt references; inspect and correct the offending data before retrying.
